...
c     Local
c
      integer i, lunw, ic(7,nexternal), n, j
      logical done
      double precision wgt,p(0:3,nexternal)
...
c
      ...
      do i=1,nw
         if (.not. done) then
            call read_event(lun,P,wgt,n,ic,ievent,scale,aqcd,aqed,done)
         else
            wgt = 0d0
         endif
      ...
      rewind(lun)
      lunw = 25
      open(unit = lunw, file='events.dat', status='unknown')
      call create_banner(lunw,swgt,nstore)
      done = .false.
      i=0      
      xtot = 0
      xover = 0
      ...
      do j=1,nw
         ...
         if (store_event(j) .and. .not. done) then
            ...
            xtot = xtot + wgt
            i=i+1
            call write_Event(lunw,p,wgt,n,ic,i,scale,aqcd,aqed)
         endif
      enddo
...
c
      integer i,j
      double precision sum_wgt,sum_wgt2, xtarget
      integer ip, np, ic, nc, jpart(7,nexternal)
      integer iseed
      double precision pboost(0:3),pb(0:3,nexternal),eta
...
c
      nc = jamp2(0)
      do ic =2,nc
         jamp2(ic) = jamp2(ic)+jamp2(ic-1)
      enddo
      xtarget=ran1(iseed)*jamp2(nc)
      ic = 1
      do while (jamp2(ic) .lt. xtarget .and. ic .lt. nc)
         ic=ic+1
      enddo
c
...
c
      ...
      enddo
      jpart(6,isym(1,jsym))=-1
      jpart(6,isym(2,jsym))=-1
c      write(*,*) 'Getting helicity',hel_buf(1:50)
      read(hel_buf,'(15i5)') (jpart(7,i),i=1,nexternal)
...
c      write(*,*) 'Writing event'
      if (q2fact(1) .ne. q2fact(2)) then
         scale = -(sqrt(q2fact(1))+sqrt(q2fact(2))/10000d0)
      else
         scale = sqrt(q2fact(1))
      endif
      aqcd = g*g/4d0/3.1415926d0
      aqed = gal(1)*gal(1)/4d0/3.1415926d0
      
      call write_event(lun,pb,wgt,nexternal,jpart,nw,scale,aqcd,aqed)
      end
