...
c                  write(*,*) "Found match graph",mapconfig(j),mapconfig(k),diff
     ...
     $     ' channels remain for integration.'
      call write_csh(mapconfig,use_config,pwidth,icomp,iforest)
      call write_input(j)
      end
...
c-----
 ...
 99   close(26)
      ...
      subroutine open_csh_file(lun)
c***********************************************************************
c     Opens csh file for looping including standard header info
...
c-----
      ...
      write(lun,15) '#!/bin/csh -f'
      write(lun,15) '#PBS -q ' // PBS_QUE
      write(lun,15) '#PBS -o PBS.log'
      write(lun,15) '#PBS -e PBS.err'
      write(lun,15) 'if ($?PBS_O_WORKDIR == 1) then' 
      write(lun,15) '    cd $PBS_O_WORKDIR'
      write(lun,15) 'endif'
      write(lun,15) 'set k = run1_app.log'
      write(lun,15) 'set script = ' // fname
      write(lun,15) 'rm -f wait.$script >& /dev/null'
      write(lun,15) 'touch run.$script'
      write(lun,15) 'echo $script'
      write(lun,'($a)') 'foreach i ('
 15   format(a)
      end

      subroutine write_csh(mapconfig,use_config, pwidth, jcomp,iforest)
c***************************************************************************
c     Writes out csh commands to run integration over all of the various
...
c        same thing as above for the csh file
c
            call bw_conflict(i,iforest(1,-max_branch,i),lconflict)
            nbw=0               !Look for B.W. resonances
            if (jcomp .eq. 0 .or. jcomp .eq. 1 .or. .true.) then
...
c      
      ...
      if (.false.) then
         ...
         write(lun,20) 'qsub public.sh -N $1$i'
      else
