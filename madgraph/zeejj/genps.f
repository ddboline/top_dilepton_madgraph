...
c         write(*,'(a,12i4)') 'Summing configs',(isym(i),i=1,isym(0))
         ...
         nbranch      = nfinal
         ndim         = 3*nfinal-4
         if (abs(lpp(1)) .eq. 1) ndim=ndim+1
         if (abs(lpp(2)) .eq. 1) ndim=ndim+1
         call set_peaks
         if (.false. ) then
...
c
      ...
      elseif (abs(lpp(1)) .eq. 1) then
         call sample_get_x(sjac,x(ndim),ndim,mincfig,0d0,1d0)
         xbk(1) = x(ndim)
      elseif (abs(lpp(2)) .eq. 1) then
         call sample_get_x(sjac,x(ndim),ndim,mincfig,0d0,1d0)
         xbk(2) = x(ndim)
      endif
...
c         call fill_invarients(nfinal,p1,stot,xx)
         if (abs(lpp(1)) .eq. 1 .and. abs(lpp(1)) .eq. 1) then
            xx(invar)=x(ndim)
            xx(invar-1)=x(ndim-1)
         elseif (abs(lpp(1)) .eq. 1 .or. abs(lpp(1)) .eq. 1) then
            xx(invar)=x(ndim)
         endif
c
...
c
      ...
      double precision smin,smax,totmass,xa2,xb2,wgt
...
c
      ...
      enddo
      if (sqrt(s(-nbranch)) .lt. totmass) then
         pass=.false.
         jac = -5d0
         return
      endif
...
c         write(*,*) 'Using s',nbranch-1+(-ibranch)*2
         ...
         if (m(ibranch-1)**2.lt.smin.or.m(ibranch-1)**2.gt.smax) then
            jac=-1d0
            return
         endif
         jac = jac * stot
...
c      if (nt_channel .eq. 0) ix=-1
      do i = -nbranch+nt_channel+1,-1         !Loop over all s-channel poles
         ix = nbranch+(-i-1)*2
