      ...
      real*8 x,Q,P2,Q2fit,f(-6:6),alfas
      nset = 1
...
c
      ...
      if (name(nset).eq.'MRSTgrid') call MRSTalfa(alfas,Q)
...
c        print *,'calling QCDNUMinit',nset,Eorder,Q2fit
...
c      print *,'entered initPDFM,',nset,imem,name(nset)
      ...
      if (name(nset).eq.'QCDNUM_MRST') then
c         print *,'calling InitEvolcePDFM (from QCDNUM_MRST)'
         call InitEvolvePDF(nset,imem)
c         print *,'calling QCDNUMpdf'
         call QCDNUMpdf(nset)
c         print *,'exitted QCDNUMpdf'
      endif
